...
2. Install development and testing dependencies:
   ...
   with option that you use and build on Windows, daily-python is not supported, you can use the following command to install the dependencies:

   ```bash
   uv sync --group dev --all-extras --no-extra gstreamer --no-extra krisp --no-extra local --no-extra ultravox --no-extra daily --no-extra mlx-whisper
   ```
